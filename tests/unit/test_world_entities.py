...
def _make_runtime(graph=None, manifest=None, demo_controller=None, copilot_service=None):
    rt = MagicMock()
    rt.world_graph = graph
    rt.world_datapack_manifest = manifest or {"warehouse_id": "DC-47", "dataset_id": "dc47-demo-v1"}
    rt.demo_controller = demo_controller
    rt.copilot_service = copilot_service
    return rt


def _make_snapshot(turn_id: str, context_snapshot_id: str, captured_at: str | None = None):
    from maiw_api.copilot.models import OperationalContextSnapshot, ContextSnapshotNode, ContextSnapshotEdge
...
class TestGetGraphEntities:
    ...
    def test_e1_returns_paginated_response(self):
        ...
        result = asyncio.run(get_graph_entities(entity_type="Worker", limit=10, offset=0, runtime=runtime))
    ...
    def test_e2_entity_type_filter_returns_only_that_type(self):
        ...
        graph, workers = _make_graph_with_workers(3)
        runtime = _make_runtime(graph=graph)
        result = asyncio.run(get_graph_entities(entity_type="Worker", limit=20, offset=0, runtime=runtime))
    ...
    def test_e3_limit_clamped_to_50(self):
        ...
        result = asyncio.run(get_graph_entities(entity_type="Worker", limit=50, offset=0, runtime=runtime))
    ...
    def test_e4_offset_paginates(self):
        ...
        runtime = _make_runtime(graph=graph)
        page0 = asyncio.run(get_graph_entities(entity_type="Worker", limit=5, offset=0, runtime=runtime))
        page1 = asyncio.run(get_graph_entities(entity_type="Worker", limit=5, offset=5, runtime=runtime))
    ...
    def test_e5_invalid_entity_type_returns_400(self):
        ...
        with pytest.raises(HTTPException) as exc:
            asyncio.run(get_graph_entities(entity_type="NonExistentType", limit=10, offset=0, runtime=runtime))
    ...
    def test_e6_graph_unavailable_returns_503(self):
        ...
        with pytest.raises(HTTPException) as exc:
            asyncio.run(get_graph_entities(entity_type=None, limit=10, offset=0, runtime=runtime))
    ...
    def test_e13_entities_route_is_get_only(self):
        ...
        route = next(
            (r for r in world_module.router.routes
             if isinstance(r, APIRoute) and "entities" in r.path and "neighbors" not in r.path and "{" not in r.path),
            None,
        )
...
class TestListContextSnapshots:
    ...
    def test_e10_sorted_newest_first(self):
        from maiw_api.routers.world import list_context_snapshots
        old_snap = _make_snapshot("turn-old", "ctx-old", captured_at="2025-01-01T00:00:00+00:00")
        new_snap = _make_snapshot("turn-new", "ctx-new", captured_at="2025-12-31T23:59:59+00:00")
