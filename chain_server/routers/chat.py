...
from chain_server.services.evidence.evidence_integration import get_evidence_integration_service
from chain_server.services.quick_actions.smart_quick_actions import get_smart_quick_actions_service
from chain_server.services.memory.context_enhancer import get_context_enhancer
from chain_server.services.memory.conversation_memory import get_conversation_memory_service
from chain_server.services.validation import get_response_validator, get_response_enhancer
    ...
    try:
        ...
        if recommendations and len(recommendations) > 0:
            ...
            user_recommendations = [
                ...
                if not any(tech_term in rec.lower() for tech_term in [
...
def _clean_response_text(response: str) -> str:
    ...
    try:
        ...
        response = re.sub(r"'response_type': '[^']*', , 'actions_taken': \[\]", '', response)
        
        # Remove patterns like ", , 'response_type': 'equipment_telemetry', , 'actions_taken': []"
        response = re.sub(r", , 'response_type': '[^']*', , 'actions_taken': \[\]", '', response)
        
        # Remove patterns like "equipment damage. , , 'response_type': 'equipment_telemetry', , 'actions_taken': []"
        response = re.sub(r"equipment damage\. , , 'response_type': '[^']*', , 'actions_taken': \[\]", '', response)
        ...
        response = re.sub(r"Following up on the previous Action: [^.]*\.\.\.", '', response)
        ...
        # Remove patterns like "awaiting further processing. ,"
        response = re.sub(r"awaiting further processing\. ,", "awaiting further processing.", response)
        ...
        response = re.sub(r"processing\. , \*\*Recommendations:\*\*", "processing.\n\n**Recommendations:**", response)
        
        # Remove patterns like "equipment damage. , , 'response_type': 'equipment_telemetry', , 'actions_taken': []"
        response = re.sub(r"equipment damage\. , , '[^']*', , '[^']*': \[\][^}]*", "equipment damage.", response)
        ...
        response = re.sub(r"over-temperature\. , , '[^']*':", "over-temperature.", response)
...
async def chat(req: ChatRequest):
    ...
    try:
        ...
        try:
            ...
            try:
                ...
                if enhanced_response.confidence_score > 0:
                    ...
                    result["confidence"] = max(original_confidence, enhanced_response.confidence_score)
                ...
                all_recommendations = list(set(original_recommendations + evidence_recommendations))
                if all_recommendations:
                    result["recommendations"] = all_recommendations
                
            except Exception as evidence_error:
                logger.warning(f"Evidence enhancement failed, using base response: {evidence_error}")
        ...
        return ChatResponse(
            ...
            recommendations=result.get("recommendations", structured_response.get("recommendations")),
