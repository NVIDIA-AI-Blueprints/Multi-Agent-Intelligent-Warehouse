...
async def run_migrations(
    ...
    try:
        ...
        if success:
            return {
                "status": "ok",
                "message": "Migrations completed successfully" if not dry_run else "Dry run completed",
                "dry_run": dry_run,
...
async def rollback_migration(
    ...
    try:
        ...
        if success:
            return {
                "status": "ok",
                "message": f"Migration {version} rolled back successfully" if not dry_run else f"Dry run rollback for {version}",
                "version": version,
                "dry_run": dry_run
            }
        else:
            raise HTTPException(status_code=500, detail=f"Failed to rollback migration {version}")
