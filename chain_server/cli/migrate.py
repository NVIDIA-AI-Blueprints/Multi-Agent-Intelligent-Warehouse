...
def load_config() -> Dict[str, Any]:
    ...
    config_path = Path(__file__).parent.parent.parent / "data" / "postgres" / "migrations" / "migration_config.yaml"
        ...
        try:
            ...
            if status.get('applied_migrations'):
                ...
                for migration in status['applied_migrations']:
                    click.echo(f"  ✓ {migration['version']} - {migration['description']} ({migration['applied_at']})")
                click.echo()
            ...
            if status.get('pending_count', 0) > 0:
                click.echo(click.style("⚠️  There are pending migrations. Run 'migrate up' to apply them.", fg='yellow'))
            else:
...
def up(ctx, target, dry_run):
    ...
    async def _up():
        try:
            ...
            if success:
                if dry_run:
                    click.echo(click.style("✅ Dry run completed successfully.", fg='green'))
                else:
...
def down(ctx, version, dry_run):
    ...
    async def _down():
        try:
            click.echo(f"Rolling back migration {version}{' (dry run)' if dry_run else ''}...")
            
            success = await migrator.rollback_migration(version, dry_run=dry_run)
            
            if success:
                if dry_run:
                    click.echo(click.style(f"✅ Dry run rollback for {version} completed.", fg='green'))
                else:
                    click.echo(click.style(f"✅ Migration {version} rolled back successfully.", fg='green'))
            else:
                click.echo(click.style(f"❌ Failed to rollback migration {version}.", fg='red'))
                sys.exit(1)
...
def health(ctx):
    ...
    async def _health():
        try:
            ...
            click.echo(f"Migration System Health - {version_service.get_version_display()}")
            click.echo("=" * 50)
...
def info(ctx):
    ...
    for migration in config['migrations']:
        click.echo(f"  {migration['version']} - {migration['description']}")
        click.echo(f"    Dependencies: {', '.join(migration['dependencies']) if migration['dependencies'] else 'None'}")
        click.echo(f"    Rollback Supported: {migration['rollback_supported']}")
        click.echo(f"    Estimated Duration: {migration['estimated_duration_seconds']}s")
        click.echo()
